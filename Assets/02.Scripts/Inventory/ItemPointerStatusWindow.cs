...
{    
    ...
    void Start()
    {        
        // 하위 오브젝트의 모든 참조는 캐릭터 Canvas의 0번째 자식인 인벤토리 오브젝트의
        // 마지막 자식인 상태창 오브젝트 참조를 기반으로 합니다.
        Transform inventoryTr = GameObject.FindWithTag("CANVAS_CHARACTER").transform.GetChild(0);
        statusWindowTr= inventoryTr.GetChild(inventoryTr.childCount-1); 
        ...
        slotListRectTr=inventoryTr.GetChild(0).GetChild(0).GetChild(0).GetChild(0).gameObject.GetComponent<RectTransform>();    
