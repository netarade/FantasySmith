 ...
 * 
 * <v2024_0116_2_최원준>
 * (이슈) 드랍시 탭정보를 얻어와서 드랍이 안되는 아이템을 구분해서 되돌려보내야함.
 * => 
 * (완료_0116)
 * 1- 다른 인벤토리 드랍시 탭이 완전히 일치하지 않으면 되돌려보내도록 하였으며, 
 * 2- IsSlotEmpty, IsSlotEnough메서드에서 내부적으로 아이템 종류에 따라 탭별 제한수를 검사하여 제한수를 넘으면 false를 반환하도록하였음.
 * (퀘스트아이템의 경우 전체탭 상태라면 GetCurRemainSlotCount메서드에서 슬롯 제한수 자체의 반환값 0이 되어 실패처리되게 하였음.)
 * 
 * 
 * 
 * (해야할 것)
 * 1- 슬롯 드랍시 퀵슬롯이라면 IsItemPlaced true로 만들기
 * 
 * 2- IsItemPlaced true일때, 같은 종류의 아이템이 한번 더 드랍 일어나면 (즉, 더미가 교체되는 순간)
 * Deactivate하여 인벤토리 창 아무곳에 넣어주기
 * 
 * 3- 한 종류의 아이템만 받게하기
 * (슬롯 드랍 시 아이템 종류와 equipWeaponType[slotIndex]가 일치해야 드롭을 허용한다.)
 * 
 * 4- 게임종료시 장착아이템을 다시 Unactivate하여 퀵슬롯에 저장한다음
 * 로드할 때 다시 장착시켜줘야 한다.
 * 즉, IsItemEquipped가 추가로 저장되고, 로드되어져야한다.
 * 
 * (슬롯에 있던 아이템은 슬롯에 자리해야 하고, 장착상태이던 아이템은 장착되어져야 한다.
 * IsItemPlaced는 슬롯에 자리한 아이템이라면 모두 true가 되어야한다.
 * 순서 - 슬롯에 포지션 업데이트 이후 장착상태의 아이템따로 보내기)
 * 
 * 
 * 
 * 
 * 
 * (이슈)
 * 1- 일단 한번 넣으면  더미셀렉트 불가능 한 상황 - 
 * 더미에 전용 셀렉트 스크립트 넣은 후 더미 셀렉팅이 일어나면, 
 * 바로 셀렉트 해제 후 - 아이템 장착 해제
 * 더미를 빈리스트로 보내고
 * 레이캐스팅 다시 쏘아서 아이템을 셀렉팅상태로 만들기
 * 
 * <2024_0117_최원준>
 * (이슈)
 * 1- 아이템 셀렉트후 다른 인벤토리로 드랍 시 (전체탭->장비탭) 드랍이 허용되지 않음
 * 
 * <2024_0118_최원준>
 * (이슈)
 * 1- Inventory클래스의 TypeMatchingItemClassToItemType메서드 확인하기 (아이템 타입 호출순서확인)
 * 저장시 세부타입으로 저장해야 하는지, 중간타입으로 저장할 수 있는지 확인하기
 * (잡화+빌딩=>잡화, 무기+방어구=>장비)
 * 
 * 2- ItemType을 ItemType.Weapon에서 ItemType.Equip으로 변경하려 했으나
 * 무기, 방어구는 따로 저장해야 하므로 대분류를 분리할 필요가 있으며, Equip이라는 추상적 단위로만 묶이게 만드는 것이 이득 
 * 
 * 
 * 3- 인벤토리 로드시 가장 가까운 슬롯부터 채워져 있는 문제 (전체탭 인덱스가 초기화됨)
 * =>(0118_해결완료)
 * DeserializeItemListToDic메서드에서 단순 AddItem메서드를 호출하고 있었는데,
 ...
 * 
 * 4- 슬롯의 더미 이미지가 슬롯에 맞춰서 축소돼야 함.
 * 현재 아이템이 UpdatePosition할 때 cellsize를 자동으로 맞추지만, 더미는 UpdatePositio메서드 호출하지 않기 때문에
 * 슬롯 드랍이 일어나는 곳에서 맞춰주어야 한다.
 * => (수정예정)모든 아이템이 최초에 한번 맞추고, 다른 슬롯으로 드롭이 일어날 때만 맞춰주고 나머지는 저장해서 써야한다.
 * 
 * 
 * (통합 이슈)
 * 
 * - 전체 -
 * 리소스 최신화가 안되어 2D 프리팹에 3D 프리팹을 달고 있었던 점
 * 
 * - Player관련 - 
 * 1- if문에서 ItemEquipSwitch문의 성공 실패조건에 따라 애니메이션 처리하기
 * 2- 장비아이템 equip시 모든 아이템 별 위치, 회전값이 다르기 때문에 아이템 정보에 STransform을 넣어서 저장해야 하는점.
 * 간단히 처리하기 위해 Visual매니저의 인스펙터뷰에 모든 Transform을 등록하고 인덱스에 따라 읽어들여 STransform으로 변환하여 생성시켜주기
 * (다음 업데이트 시 반영해야할 사항)
 * 
 * STransform 및 ItemWeapon 생성자에 STransform정보 추가
 * 
 * 
 * -빌딩아이템 관련-
 * CreateManager.cs 
 * 1- 오브젝트 생성시 태그를 교체하지 않도록 해야함
 * 2- 오브젝트 생성시 부모쪽에 콜라이더가 없어도, 자식쪽에 콜라이더가 하나라도 있다면 대체하지 못하도록 해야함.
 * 
 * ItemInfo4.cs
 * 1-Durability 감소시 itemBuilding 조건문이후로 itemWeapon을 호출하고 있던 점.
 * 
 * ItemInfo3.cs
 * 1-CheckDestroyInfo에서 2D 상태에서의 파괴와 3D상태에서의 파괴를 구분하지 않고 있었던 점
 * 
 * (수정 예정)
 * 1- 섭취아이템 우클릭시 모든 인벤토리를 향해 레이캐스팅하는 메서드를 만들고 호출해주기. (InventoryInfo 및 PlayerInteractive)
 * 
 * <2024_0122_최원준>
 * (구현 예정)
 * 1- 인벤토리 (퀘스트아이템을 제외한)모든 탭 슬롯제한 공유옵션 - 전체탭으로 통일
 * 
 * 2- 퀵슬롯 해당장비류만 들어갈 수 있게 (완료_0123)
 * 
 * 3- 외부장식용 빌딩아이템의 월드저장 (STransform활용, 월드인벤토리)
 * 
 * 4- 무기 아이템 장착시 기본콜라이더 꺼주기. 장착해제 시 켜주기  (완료_0123)
 * =>(무기 드롭시에 필요한 콜라이더가 장착할 때 켜져서는 안됨.)
 * 
 * 5- 무기 고정 슬롯 (타입에따라 슬롯이동못하게 막기) (완료_0123)
 * 
 * 6- 셀렉트 제대로 이뤄지지 않는 부분 수정하기 (드랍해도 다시 셀렉트 되는부분) (부분완료 최적화필요_0123)
 * 
 * 7- 퀘스트아이템 셀렉트막기
 * 
 * (수정내용)
 * 1- ItemMisc.cs - 생성자에서 중첩수량을 0에서 1로 만들어놓았던 점 수정(아이템 생성시 추가로 1개들어가버림)
 * 2- Inventory_3.cs - IsEnough메서드 로직 작동 오류 수정 (IsExist에서 count로 바로호출했던점 수정)
 * 3- ItemOthers.cs - 빌딩아이템 isDecoration속성추가
 * 4- CreateManager.cs - 빌딩아이템의 isDecoration속성에 따라서 태그를 붙이도록 변경
 * 5- InventoryInfo_3.cs - 그래픽레이캐스팅 기능을 ItemSelect에서 InventoryInfo가 보유하고 있도록 변경.
 * 6- ItemSelect.cs - 그래픽레이캐스팅을 InventoryInfo에서 호출하도록 변경.
 * 
 * 
 * <v2024_0123_최원준>
 * (수정내용)
 * 1- GetItemObjectList 메서드 내부에 바로 아이템 이름으로 딕셔너리 접근 코드를
 * 인덱스를 먼저 구하여 예외처리하도록 하여 
 * GetItemObjectList를 사용하는 모든메서드의 코드 안정성을 높임.
 * 2- 클라이언트 인벤토리 레이캐스팅 시 서버 레이캐스팅을 이용하도록 변경
 * 3- 타 인벤토리 슬롯 드롭 조건 해당타입을 담을 수 있어야 하는 추가
 * 
 * (수정예정)
 * 1- 타 인벤토리 슬롯으로 아이템 디셀렉트 이후 바로 셀렉트 되는 현상
 * 인벤토리의 상태가 한쪽인벤토리에만 있기 때문.
 * 즉, 슬롯을 옮길 때 상태도 전이시키거나 새롭게 설정하여 특정 시간 이후 비활성화하도록 해야 함.
 * => (완료_0123)
 * 
 * <2024_0123_최원준>
 * (수정내용)
 * 1- 타 인벤토리 슬롯으로 아이템 디셀렉트 이후 바로 셀렉트 되는 현상 1차 해결완료
 * 2- 무기 슬롯간 이동 막기, 장착시 콜라이더 해제
 * 3- STRansform에 스케일 관련 변수와 메서드를 추가, VisualCollection에 equipTr을 추가
 * 
 * 
 * (구현예정)
 * 1- 월드의 빌딩 아이템 위치가 저장되어져야 함.
 * => 아이템 저장 시 계층구조를 변경하면 안되고 그대로 저장해야 하며, 
 * GameObject가 아니라 ItemInfo를 저장해야함.
 * 이유는 오브젝트 자체를 저장할때 현재 2D로 저장하기 때문에 내부 ItemInfo의 참조 위치가 틀려짐.
 * 로드할 때 2D에 있는것이 아니라 3D에 배치해줘야 함.
 * 
 * 2- 월드의 보관함 위치가 저장되고 로드 되어야 함.
 * 
 * 3- 드랍할때의 Transform 정보에서 rotation과 scale을 자기 프리팹(3D 오브젝트에서)받아서 해당 정보대로 그대로 떨어지기
 * (포지션값만 조절가능하게)
 * 
 * 4- 장착할 때 position, rotation, scale을 ItemInfo의 STransform을 참조하여 결정하기
 * 
 * 5- ItemFood VisualManager Index설정해서 CreateManager와 연동
 * 
 * <2024_0124_최원준>
 * (추가 구현예정)
 * 1- 탭 프리팹, 슬롯 프리팹의 애니메이션 및 폰트, 컬러설정에 따라 만들어둔 것을
 * 동적할당이 아니라 셀렉팅 (활성화,비활성화 방식으로 변경하기)
 * 
 * (이슈)
 * 1- 월드에 저장하는 아이템의 경우 어떤 플레이어가 만든 빌딩 아이템인지 저장해야함.
 * 모든 설치형 빌딩아이템은 만들어질 때 소유자정보가 있어야 한다. 
 * (인벤토리에 들어온 순간 해당아이템은 playerId를 부여받아야 함)
 * 
 * 2- 플레이어가 이동할때 아이템셀렉팅이 계속일어나는 문제
 * 인벤토리에 담긴아이템은 인벤토리창이 닫혀있을 때 CanvasGroup으로 인하여 셀렉팅이 일어나지 않아야함.
 * => 퀵슬롯의 더미아이템 셀렉팅을 확인하였고, 월드아이템 셀렉팅이나 인벤토리 아이템 셀렉팅여부를 확인하여야함. 
 * 
 * 3- 보관함을 동적으로 생성시켜야 한다.
 * CreateManager에서 새로운 생성메서드를 만들고 소유주 ID를 다르게해서 생성해야.
 * (보관함을 저장할때 보관함의 소유주도 설정되어져야 한다.)
 * 
 * 4- 보관함이 재료를 소모한 후 설치형아이템으로서 설치 후 인벤토리에 들어올 일은 없지만, 아이템처럼 무기를 통해 파괴가 가능하다.
 * => 보관함도 아이템화해야 하고, 고유의 세이브파일명을 저장하고 있어야한다.
 * 보관함이 부서질때 계층구조가 월드상태오브젝트와 같아야 durability소모로 부서질 수 있다.
 * 
 * 5- 월드아이템 로드시 오브젝트를 생성해주는 기능이 필요함. 즉, 내부정보 로드후 월드에 오브젝트를 생성해주어야 함.
 * => ItemBuilding의 경우 isDecoration 속성이 걸린 아이템의 경우 STransform 정보가 저장되어져야 함.
 * 
 * 6- 설치완료후 저장되기 직전에 STransform을 기록하고 저장되어져야 함.
 * 
 * 
 * 
 * (수정사항)
 * 1- Inventory클래스의 딕셔너리 타입을 GameObject에서 ItemInfo 기반으로 변경하여
 * Inventory, InventoryInfo, InventoryInteractive의 각 연관메서드를 수정
 * 
 * 2- 아이템 속성에 소유주 ID를 저장하도록 설정
 * 
 * 3- 인벤토리의 소유주ID를 인벤토리 계층 최상위 오브젝트명으로 설정하고,
 * 아이템 추가 및 제거 시 인벤토리 소유주ID에 따라서 아이템의 소유주ID가 변동되도록하였음.
 * 
 * 4- 아이템 생성시 소유주 ID가 World로 설정된 상태로 생성
 * 
 * 5- ItemBuilding의 STransform worldTr속성 추가
 * 
 * 
 * <2024_0125_최원준>
 * (수정사항)
 * 1- 고유 넘버를 부여하는 기능
 * 2- 인벤토리의 종류 구분
 * 3- 이니셜라이저에서 고유넘버를 저장받아 초기화
 * 4- 3D오브젝트의 위치값을 반영하는 기능
 * 5- 빌딩아이템의 isDecoration속성을 삭제하고 enum으로 변경
 * 6- 기타 ItemInfo의 프로퍼티 수정 및 추가
 *  
 * 
 * (구현예정)
 * 1- 상태창 텍스트를 현재 프로젝트 데이터에 맞게 한글이 아니라 영어로 수정해주기
 * 
 * (이슈)
 * 1- 빌딩아이템을 생성할 때 월드 인벤토리에 createManager에서 등록할 것인지,
 * 아니면 생성하는쪽에서 Register를 통해 월드인벤토리에 등록하게하고, 고유Id를 부여할 것인지
 * 
 * 
 * 2- 인벤토리 설치 시 고유Id는 새롭게 받아야하고, 플레이어 Id는 소유자 ownerId로 저장하고 있어야 한다.
 * 빌딩아이템의 속성으로 id를 추가해야한다.
 * => Register할때 플레이어 정보를 받아야 한다.(소유자 Id를 결정하기 위해)
 * 
 * <2024_0125_최원준>
 ...
 * 
 * (이슈)
 * 1- IdType을 Inventory말고 User를 추가로 설정하였으며, Item클래스에 ownerId이외의 id변수를 추가하였음.
 ...
 * (이슈) 
 * 
 * 1-아이템이 인벤토리창을 닫아도 키보드로 셀렉팅 일어나는 현상
 * => ItemInfo의 SwitchAppearAs2D메서드를 public 으로 설정하여 인벤토리 창을 닫을 때 
 * 인벤토리가 호출하도록 하여 아이템 셀렉팅을 막음.
 * 
 * 2- 월드의 아이템이 저장될 때 한곳으로 모이는 현상
 * 
 * 3- 무기를 장착하고 더미 아이템의 이미지에 커서를 갖다 대게 되면
 * OnPointerEnter OnPointerExit 콜백메서드에서 
 * statusInteractive.OnItemPointerEnter(this)를 호출하게 되어 null레퍼런스가 뜨는 현상.
 * 
 * 더미 아이템은 단순 이동만 했기 때문에 statusInteractive를 따로 잡아주지 않았으며,
 * 원래는 장착한 아이템의 정보를 보여 줘야 함.
 * 
 * 4- 무기 장착 시 스케일 값이 이상하게 되는 현상
 * 
 * 
 * (완료)
 * 1- OnItemWorldDrop 시 기본적으로 전달한 변환정보의 위치 값만 적용시키고 인자를 통해 추가적으로 적용가능하도록 변경
 * (드랍 시 기본 위치와 스케일 정보대로 그대로 떨어지도록)
 * 2- InventoryInfo - 동일한 이름의 아이템 갯수를 알려주는 HowManyCount메서드 구현
 * 3- 인벤토리를 Off할 때 모든 아이템의 셀렉팅을 막을 수 있게 SwitchAllItemAppearAs2D 메서드 구현
 * 4- 더미아이템의 포인터 콜백메서드 호출을 막음
 * 
 * (구현 예정)
 * 1- 아이템화 인벤토리 Durability 0이 될때 ID등록을 해제할 수 있도록
 * (파일 삭제를 못하므로 새로운 파일을 만들어 로드를 막아야 한다.)
 * 2- 음식 섭취
 * 3- 애니메이션 만들어둔 탭 및 슬롯을 동적할당시키기
 * 4- 탭 슬롯 제한 공유
 * 
 * <2024_0127_최원준>
 * (이슈)
 * 1- 아이템 장착 해제 시 item2dTr이 스케일값을 그대로 가져가는 현상발생. (이미지 크기가 바뀜)
 * => 2D상태에서 2D하위에 있는 item3dTr의 스케일이 장착 시 0.3->1로 바뀌고 (3배)
 * 돌아올 때 원래의 정보를 넣어주지 않고 다시 item2dTr하위에 item3dTr을 넣으면
 * 자식 item3dTr의 스케일에 따라 item2DTr의 스케일이 바뀜. (3배가됨)
 * 
 * 2- 장착할때만 item3dTr의 위치정보를 변경하고,
 * 장착해제가 이루어질 때 item3dTr의 위치정보를 다시 입력해주지 않았는데도 불구하고
 * item3dTr의 위치값이 원상복구 되어있음
 * 
 * => 이는 item2dTr이 자식의 정보를 기억하고 있어서, 다시 하위자식으로 돌아올때 위치정보를 넣어주는 것으로 보임.
 * 또한 자식 스케일이 달라져 있으면 이에 영향을 받는것으로 판단됨.
 * => (정정) 아이템 장착시 item3dTr의 위치, 회전, 스케일값을 변형시키고,
 * 다시 해제 시 item2dTr하위로 들어올 때 item3dTr 위치값은 임의의 값이 되어버리고, 회전값은 원래대로 돌아옴.
 * 스케일은 변경되지 않지만, 부모의 스케일이 자식에 영향을 받아 변하게됨. 
 * (그래서 절대 스케일은 조절못하게 하는 것인지도 모름)
 * 
 * 3- STransform의 값만 복사하여 새로운 인스턴스를 반환할 수 있도록 Clone인터페이스를 구현하였음.
 * (Item 내부에 Stransform을 저장하고 있는 경우 참조값을 저장하기 때문에, 아이템이 Clone되어도 참조값을 공유하게 되어있음.
 * 따라서 새로운 STransform의 Clone도 같이 교체해서 반환해줘야함.)
 * 
 * 
 * 
 * => (해결) item2dTr의 스케일이 바뀌어 있는 이유는 아이템 장착 시 item2dTr이 item3dTr의 하위에 종속되기 때문에
 * item3dTr이 3배가 된다면 item2dTr도 3배가 될 것이고,
 * 장착 해제 할 때는 item3dTr을 item2dTr의 하위 자식상태에서 변환정보를 원상복귀해주기 때문에 부모의 스케일이 변하지 않는것.
 * 이를 해결하기 위해 item3dTr의 스케일이 3배가 될때 item2dTr의 local스케일을 다시 1로 만들어줘야 한다.
 * 
 * (완료내역)
 * 
 * 1. 인벤토리에 동일한 이름의 아이템 수량검색기능 추가 (HowManyCount 메서드)
 * 
 * 2. 아이템 착용 시 저장되어있는 착용 정보에 따라 개별적으로 아이템 장착이 가능하도록 구현
 * 
 * 3. 아이템 착용 시 이미지 스케일이 같이 변경되는 문제 수정
 * 
 * 4. 월드아이템의 위치정보 저장이 동일한 인스턴스를 공유하여 같은 위치로 저장하는 문제를 해결
 * (ItemBuildng클래스 Clone 메서드 오버라이딩)
 * 
 * 5. 아이템이 인벤토리 창을 닫아도 키보드로 캐릭터 이동 시 셀렉팅이 되어 자동드랍이 이루어지는 부분 수정
 * (인벤토리가 닫힌 상태에서 모든 아이템의 개별 레이캐스팅을 막도록 개별 메서드를 호출)
 * 
 * <2024_0128_최원준>
 * (이슈)
 * 1- 아이템 생성버튼을 중복해서 눌릴 때 칸이 없는데도 계속 생성되는 문제 (슬롯 0번에 겹치는 문제) (완료_0128)
 *  
 * 2- 아이템 줍기할 때 2D 이미지가 회전하는 문제 (아이템 3D의 회전값을 받아오는 문제)
 * (완료_0128) UpdatePositionInfo메서드 내부에서 itemRectTr의 절대회전값을 초기화하고 있었는데, 상대회전값을 초기화 해야함
 * 
 * (이슈)
 * 1- AddItem시 아이템 생성할 수 있는 칸이 없을 때, 잡화를 최대 수량까지 더 채울 수 있는데 딱 맞춰서 생성하면 못채우는 문제
 * 2- AddItem시 슬롯에 빈자리가 없는데 잡화아이템의 경우 0번 슬롯에 채워지는 문제
 * =>(완료_0128) 
 * 잡템이 슬롯은 더 없지만 최대수량까지 더 채울 수 있는 경우에 인벤토리에 들어갈 수 있어야 하는데, 
 * InventoryInfo클래스에서는 IsAbleToMisc으로 Add명령이 통과되었으나 Inventory클래스의 AddItemMisc에서는
 * GetCurRemain슬롯으로 조건검사하여 통과하지 못하여 인벤토리에는 추가하지 못하고 아이템 오브젝트가 생성된 상태로 업데이트가 이루어져 발생한 문제 
 * 
 * 
 * <2024_0129_최원준>
 * (구현 예정)
 * 1. 종료 시 - 무기 퀵슬롯 해제 (완료_0129 - 테스트필요)
 * 2. 장착 시 중력 해제, 장착 해제 시 중력 적용시키기 (완료_0129)
 * 3. 아이템 창 퀘스트 아이템 후드 착용 (퀵슬롯 미사용, 이미지는 이동하지 않음)      (완료_0130)
 * 착용 시 PlayerStatus의 isHood - true로 만들기
 * 해제 시 false로 만들기 
 * 4 상태창, 인벤토리탭 설명 영어로하기 (완료_0130)
 * 5. 나머지 인벤토리 탭 제거 옵션 넣기 (이미 완성되어있음. Tab설정을 하지 않으면됨.)
 * 
 * 6. 탭 공유  (완료_0129)
 * 슬롯제한 수 반환할 때 전체탭으로 반환 
 * 인덱스도 전체 탭 기준으로만 설정
 *  
 * 7. 푸드 먹기 안될때 있음.(형변환 실패)
 * => (완료_0129)
 * 생성했을때 ItemFood로 생성되지만, 저장할때 ItemMisc으로 저장하기 때문에, 로드했을때 ItemMisc으로 들어가서 그런 것.
 * 
 * 
 * 
 * (탭공유 구현 이슈)
 * 1- 탭공유 시 슬롯의 숫자는 전체탭과 동일하다.
 * 2- 아이템이 Add될때는 Type에 따른 슬롯제한 수를 읽는데 어떤 타입이든 전체 슬롯 제한수를 반환받는다.
 * 인덱스를 설정할 때 무조건 전체 인덱스 기준으로 설정한다. (개별인덱스는 전체 인덱스와 동일해진다.)
 * 만약 개별탭을 클릭했을 때는 슬롯의 갯수는 전체 갯수와 같고, 인덱스는 전체 인덱스기준으로 보여준다. (개별인덱스가 어차피 전체인덱스 기준으로 들어있다.)
 * 3- 오브젝트 카운팅을 전체탭 기준으로 카운팅하고, 남은 슬롯 수도 전체탭에따라 비교해야한다.
 * 
 * 4- 전체탭으로 공유를 하게 되면 아이템이 Add될 때 개별탭 오브젝트수만 단독으로 증가시키면 안된다.
 * 아예 전체 탭 오브젝트 수만 증가시켜 관리하거나, 개별탭 오브젝트수를 증가시킬 때 다른 개별 탭 오브젝트 수도 이에 영향을 받아 증가해야 한다.
 * (즉, 전체탭 오브젝트수와 개별탭 오브젝트 수가 구분없이 동기화되어야 한다.) 
 * 
 * 
 * 5- 슬롯 드롭시 인덱스가 바뀌지 않는문제 (개별인덱스 혹은 전체인덱스 1개만 바뀌는 문제)
 * 
 * 
 * 
 * 
 * (이슈)
 * 1- 하위 자식으로 관리하다가 저장할 때 상위부모 클래스로 저장하게 되면 로드할 때 하위자식변환이 불가능해지는 이슈가 있음.
 * => ItemFood의 경우 ItemMisc에서 관리하지만 저장할 때는 ItemFood따로, ItemMisc은 ItemFood를 제외하고 저장해야함. 
 * 
 * 2- 중첩아이템의 경우 수량 소모로 인해 중첩이 가능한 상태이면, 
 * 세이브할 때 중첩하지 않고 저장해놨다가, 로드할 때 강제로 중첩되어 오브젝트가 사라진것 처럼 보이는 이슈가 있음.
 ...
 * 
 * 4- ItemInfo.cs 
 * MoveSlotToSlotSameInventory메서드 - InventoryInfo의 IsShareAll에 따라 슬롯 드롭 시 인덱스 결정
 * 
 * 5- Inventory.cs
 * TypeMatchingItemClassToItemType 메서드 - ItemFood를 검사해서 반환하도록 구현 
 * DeserializeItemListToDic메서드 - AddItem시 중첩하지 않고 추가하도록 변경
 * 
 * 6- SaveData_Inventory.cs
 * ItemFood 리스트를 개별적으로 저장했다가 로드 시 ItemMisc에 합쳐주도록 하였음. 
 * 
 * 7- 
 * ItemWeapon.cs - 장착 관련 STransform 속성을 ItemEquip으로 이동
 * ItemOthers.cs - ItemQuest를 ItemEquip을 상속하게 하였으며, 장착 관련 속성추가
 * WorldItemData_Quest - 장착관련 데이터 추가
 * WorldItemData_Weapon - 장착관련 데이터 추가
 * 
 * 
 * <2024_0130_최원준>
 * (수정내역)
 * 1- 더미이미지를 인벤토리에서 슬롯 갯수만큼 생성하던 부분을 삭제하고 아이템마다 더미 이미지를 하나 보유하고 있도록 변경
 * (Inventory 오브젝트 하위 emptyList에 있던 더미 프리팹을 지우고 Item2D 프리팹하위로 이관)
 * - 코드를 일관성있게 작성하기 위함
 * 
 * 
 * (보관함테스트)
 * IsEnoughToBuild 주석처리하고 테스트 (완료_0130)
 * 
 * (통합 - 추가구현)
 * 1- 슬롯에 장착했을 때 퀵슬롯 Icon의 이미지 꺼지게하기. (로드시에도)
 * 2- 퀵슬롯 상태창 이름떠야할 자리에 설명떠있음. (퀵슬롯만)
 * (완료_0130) 계층구조 문제
 * 
 * (확인할것)
 * 1- 보관함 상자 한번 열고 두번째 안열어지는 버그
 * => (완료_0130) 아이템이 담기게되면 ItemInfo가 중간 계층에 많아지게 된다. 
 * 원래 보관함의 ItemInfo는 마지막 인덱로 ItemInfo를 직접참조하는 것으로 변경
 * 
 * (해야할 것)
 * 더미 이미지의 상태창 보여주기(완료_0130)
 * 
 * (이슈) 
 * 1- 더미 이미지가 겹쳐있을 시 원본 아이템 이미지 크기가 변경되는 문제
 * => (완료_0130) 더미 이미지를 사용하지 않을때는 Image를 Off해놓아야 함
 * 
 * 2- 방어구 장착 같은 경우 드롭되는 오브젝트와 장착 오브젝트가 틀림.
 * => 드롭되는 오브젝트를 장착하는 것이 아니라 플레이어 쪽에 숨겨놓은 오브젝트를 활성화, 비활성화 시켜야함.
 * 
 * 3- 로드 시 장착을 못하게 막아두어야 함.
 * 이유는 플레이어쪽에 상태를 넣어두어야 하는데 메서드화하기 복잡하기 때문에, 세이브할 때 장착해제 상태로 세이브해야 한다.
 * 
 * 4- 아이템 장착 해제 시 버튼 깜빡임 문제
 * => (완료_0131) 버튼이 셀렉팅 해제가 될 때 Disabled Color가 Normal Color와 일치하지 않아서 생기는 문제
 * 
 * 5- 아이템 보관함 로드 시 ownerID를 수동으로 잡아줘야 하는 부분을 빠트려서 저장이 제대로 되어도 (owenerID초기값: -1)다른 파일을 로드해버리는 현상이 발생.
 * => (완료_0131) CreateWorldItem의 ItemInfo 오버로딩 메서드에서는 새로운 아이템 생성 시 새로운 ID를 할당하고 업데이트 해주었지만
 * 로드시에는 CreateWorldItem Item 오버로딩 메서드가 작동하기 때문에 기존 ID를 업데이트해주는 부분을 빠트린것.
 * 
 * 
 * <2024_0210_최원준>
 * (수정내역)
 * InventoryInfo.cs
 * 1- 기존 inventory, initializer, interactive public 변수를 protected 변수로 변경하고 읽기전용 프로퍼티로 두는 형태로 관련 코드 수정
 * 2- 저장 및 로드 시 반드시 슬롯번호를 지정하도록 매개인자 추가
 * 3- 로드 시 PlayerPrefs의 세팅값으로 받은 값을 슬롯번호 인자로 넣어 호출하면 해당 파일이 로드되도록 코드 수정
 * 세이브 시 더 이상 종료 시 자동 호출하는 것이 아니라 UI이벤트 핸들러에 연결하여 버튼을 눌렀을 때만 저장되도록 변경 
 * 
 * 4- InitOpenState메서드의 호출을 인벤토리 로드 이후로 설정 (기존 Awake에서 Start로 변경)
 ...
 * 
 * (이슈)
 * 1- 다른 인벤토리로 슬롯 드롭 시 아이템 스위칭이 발생하면 다른 아이템이 셀렉팅되는 문제가 있음
 * 현재 드롭 바로 직전 다른 인벤토리의 레이캐스팅을 막아서 모든 아이템의 셀렉팅이 일어나지 않게 하였는데 실패하였으며
 * 이는 인벤토리의 캔버스 그룹을 막아도, 아이템 개별적인 캔버스그룹이 영향받지 않기 때문인것으로 생각됨
 * 
 * 2- 드롭 성공 시 이전 인벤토리에 전체 셀렉팅 상태를 새로운 인벤토리에 전이 시켜 준 다음, 기존 인벤토리의 셀렉팅 상태를 다시 원상복귀해주어야 하는데
 * 코드를 잘못적었으나 아직까지는 문제가 발생안함
 * 
 * (해야할 것)
 * 1- 퀵슬롯 상태창 디자인 맞추기
 * 2- 상태창 설명 영어 ->한글 변경
 ...
 * (수정내역)
 * ItemInfo.cs 
 * 1- EquipSwitch 통합 메서드 호출을 통해 착용타입을 분기해서 전송 장착 메서드와 숨김 장착 메서드를 분기해서 호출해주고,
 * 현재 착용상태를 관리하며, (로드 옵션을 선택인자로 넣어서) 로드 시에는 착용상태를 반영하지 않고 강제로 착용시킬 수 있게끔 구현
 * 
 * 2- EquipAction 대리자 관련 코드를 삭제
 * 이유는 아이템 착용 시 플레이어 관련 추가 액션을 실행해주기 위한 용도로 사용하고 있었는데 
 * Equip을 2종류로 구분하면서 숨김 장착에서는 대리자 액션을 따로 등록할 필요없이 플레이어 정보를 참조하여 장착 메서드를 호출해야하고,
 * 전송 장착에서는 로드시에만 대리자 액션을 호출해주면 되는데,
 * 
 * 이를 게임 진행 중에 계속 들고 있거나, 복잡하게 코드로 분기하기보다
 * 각 개별 착용 메서드에서는 상태를 건드리지 않고 아이템의 착용만 담당해주고 
 * 플레이어 상태는 로드할 때만 예외적으로 다른 스크립트의 설정 메서드를 호출해주거나, 인벤토리에서 호출해주는 것이 효율적이기 때문 
 * 
 * 
 * 
 * 
 ...
 * 
 * <2024_0215_최원준>
 * 1- InventoryInfo.cs 및 WorldInventoryInfo.cs, CreateManager.cs에 존재하는 파일 세이브 로드를
 * 오토 세이브 로드가 아닌, 지정 슬롯 세이브 및 지정 슬롯 로드로 변경
 * 
 * 2- 로드시 아이템 장착이 유지할 수 있도록 기능  구현
 * 
 * InventoryInfo.cs
 * - 로드 후 가시성을 표현하는 메서드에서 아이템의 IsEquip 상태를 강제로 false로 만드는 메서드 제거 및 EquipSwitch 통합메서드 호출
 * 
 * ItemInfo.cs
 * 아이템 장착 관련하여 통합메서드 호출로 변경
 * 아이템 장착 메서드 호출 시 더미 관련 정보 업데이트를 DummyInfo 통합메서드 호출로 대체
 * 더미 아이템 셀렉트 스크립트 추가
 * 더미 아이템에 버튼 추가 (DisabledColor 흰색설정)
 * 
 * 
 * 3- 퀵슬롯 선택 시 장착을 강제로 해제할 수 있도록 구현예정
 * 
 * 4- 아이템 스왑 시 장착을 강제로 해제하고 스왑 되도록 구현예정
 * 
 * 
 * 
 * 
 * 
 * (이슈)
 * 1- 아이템이 착용상태가 되었을때 2D 이미지가 분리되어서 보여지는 버그가 있음
 * (3D오브젝트는 플레이어 계층에 가지만, 2D 오브젝트가 3D오브젝트 하위에 속하지 않고
 * 
 * 3D 상태(착용상태)일 때 2D 이미지 레이캐스팅을 종료하는 것이 아니라 
 * 추가로 이미지 컴포넌트를 비활성화하는 방법
 * (하지만 왜 분리가 일어나는지 알아야 하므로 아직까지는 보류)
 * => 로드시 OnItemAdded에 의해 UpdatePositionInfo가 작동하는데 해당 코드 내부에 착용 상태의 아이템의 경우 더미의 UpdatePositionInfo만 작동하도록 막아두었기 때문에
 * Item2dTr이 최상위 계층에 생성된 상태 그대로 존재하기 때문
 * 
 * 2- 더미 아이템 셀렉트를 할 때 더미 오브젝트의 셀렉팅을 종료하고
 * 실제 오브젝트의 셀렉팅으로 대체해야 하는데
 * 실제 오브젝트의 OnSelect만 호출하면 다른 콜백메서드로 연계해서 이어지지 않음.
 * =>EventSystem.current.SetSelectedGameObject 메서드를 호출하여 셀렉팅 오브젝트를 직접 수정해줘야 함.
 * 
 * 3- OnSelect에서 EventSystem.current.SetSelectedGameObject(null);메서드를 호출하여 셀렉팅을 강제로 해제하는 경우 셀렉팅 상태에서 셀렉팅을 시도했다고 오류가 발생 
 ...
 * 
 * 4- 더미아이템 셀렉팅 시 장착해제후 기존아이템 재셀렉팅이 일어날 때, 셀렉팅 이미지 위치가 어긋나 있는 점이 있음
 * => 이는 기존아이템이 UpdatePositionInfo가 작동하지 않아서 퀵슬롯으로 이동하지 못한 상태로 셀렉팅이 일어나서 아이템을 착용했을 때의 위치로 가서 셀렉팅이 일어나기 때문 
 * 
 * 
 * <2024_0218_최원준>
 * (이슈)
 * 1- 슬롯 이동 시 탭 동일 여부 구분은 의미가 없음. 실제 담을 수 있는 아이템 종류가 일치하는 지 봐야함.
 * +동일 탭+
 * [장비탭]->[장비탭]
 * (무기)		 (방어구)
 * => 실패
 ...
 * 
 * 
 * 2- 한 쪽이 전체 탭 상태가 틀릴 때, 드랍하는 쪽의 전체 탭 여부에 따라서 인덱스 처리가 달라져야함.
 * [장비탭]->[전체탭]  
 * (방어구)    (방어구)
 * => 드롭 가능, 전체탭 인덱스 처리
 * 
 ...
 * 
 * 
 * 3- 사전이 존재하여 다만 담길 수 있는 아이템이라도, 옮겼을 때 가시성을 보여주는데 지장이 있다면 실패처리를 해야 함.
 * (둘다 개별탭이라면, 탭일치가 되어야 함)
 * [장비탭] -> [잡화탭]
 * => 실패처리
 * 
 * <2024_0220_최원준>
 * (이슈)
 * 1- 슬롯에 직접 드롭할 때 AddItemToSlot메서드를 인자로 해당 슬롯 인덱스와 탭상태를 받아서 호출하는데 
 *  전체탭 상태일 때는 개별 인덱스를 지정하고 전체 인덱스는 남는 자리 할당, 
 *  개별탭 상태일 때는 전체 인덱스를 지정하고 개별 인덱스는 남는 자리 할당하는 형태로 구현했었음.
 *  
 * 하지만, 스위칭할때는 기존에 가지고 있던 인덱스 2개를 모두를 지정해서 Add처리를 해야함.
 * 즉, 지정 슬롯 인덱스로 아이템을 추가하는 경우
 * 활성탭 인덱스하나만 지정해서 추가하고 비활성탭 인덱스를 할당받는 경우와
 * 개별탭 인덱스 및 전체탭 인덱스를 모두 지정해서 추가하는 경우가 있어야 함.
 *  
 *  
 *  
 * 2- 다른 인벤토리 이동 조건에서 (두 인벤토리 모두 아이템을 담을 수 있는 사전이 존재한다고 가정할때)
 * a - 둘다 개별 탭이면서, 개별 탭이 일치하지 않는다면 가시성이 보장되지 않으므로 실패처리
 * (스위칭이나, 간접추가를 허용해주지 않음)
 *  
 * b- 한쪽이 전체탭이고 다른한쪽이 개별탭이라면, 
 * 개별탭->전체탭으로는 가시성을 보장해줄 수 있어서 슬롯이동가능
 * 전체탭->개별탭으로 슬롯이동을할 때 (아이템과 일치하는 정확한 탭이 아니라면 실패처리를 해야함.)
 ...
 *  (수정 사항)
 *  Inventory_2.cs 
 *  1- SetItemSlotIndexIndirect, SetItemSlotIndexDirect메서드에서 퀘스트아이템에 따른 인덱스 지정 분기코드 수정 
 *  2- FillExistItemOverlapCount메서드내부의 FillCountInLoopByOrder메서드에서 아이템을 채우고 난 후 채운 수량만큼 감소 시켜야 하는데 증가 시키고 있던 부분 수정
 *  
 *  InventoryInfo_2.cs
 *  1- AddItemMisc, AddItemToSlot메서드에서 IsAbleToOverlap 옵션을 추가하여 잡화아이템 추가 시 겹치지 않고 오브젝트 그대로 추가할 수 있게 하였음.
 *  2- IsSlotEmpty, AddItemToSlot, AddItemMisc, AddItemBasic메서드에서 기존에 전체탭 상태변수를 인자로 받던 것을 삭제하고, 활성 슬롯 인덱스만 전달받도록 설정
 *  3-AddItemToSlot메서드의 활성 슬롯인덱스와 비활성 슬롯인덱스 모두를 인자로 전달받는 오버로딩 메서드 추가
 *  (기존 활성슬롯인덱스만 전달받는 메서드의 경우 비활성인덱스를 간접할당받았으나, 스위칭하는 경우에는 비활성인덱스도 지정해서 슬롯에 넣어줘야 하기 때문)
 *  
 *  InventoryInteractive.cs
 *  1- IsAbleToAddAnotherInventoryItem 신규메서드추가 - 다른 인벤토리에서의 슬롯 이동가능성 여부를 판단하는 메서드 (두 인벤토리 사전 및 탭상태에 따라서 조건 비교)
 *  
 *  
 *  
 *  
 *  <2024_0221_최원준>
 *  (이슈)
 *  1- 아이템 셀렉트 후 드롭 시 현재 상태 변수 조절을 통하여 무조건 다음 셀렉팅을 막고 있는데 잡화아이템 채우기 드롭 같은 경우는 셀렉팅을 그대로 이어줘야 하는 경우가 생기게 됨.
 *  다시 셀렉팅을 해줄 때 이벤트 시스템의 SetSelectedGameObject 메서드를 호출해줘야 하는데, 이 때 셀렉트 상태초기화를 즉시 진행 후 재 셀렉팅을 해줘야함.
 * => 아이템 셀렉트 스크립트의 DropWithRayCast메서드 호출이 끝나고 InitLastOnDeselect메서드가 자동호출되면서 코루틴으로 상태를 넣어주는데
 *  SetSelectedGameObject 메서드가 일어나는 지점은 셀렉트 스크립트가 아이라 DropWithRayCast메서드 내부의 ItemInfo스크립트의 메서드 내부에서 일어나므로 코루틴을 즉시 넣어주고 중단시켜줘야함.
 * => 코루틴 시작을 안했으므로 중단을 하지 못함. 아이템의 인벤토리 드롭메서드가 끝난 이후에 자동으로 코루틴이 호출될 수 밖에 없으므로, 
 * 아이템 셀렉팅 스크립트에 재셀렉팅을 요청하는 메서드를 만들어야함.
 *  
 *  
 *  2- 캐릭터 이동 시 퀵슬롯 아이템의 셀렉팅이 이루어지는 문제
 * 달리는 상태에서 셀렉팅을 막거나, 더미 이미지의 버튼 자체를 없애거나, 인벤토리 닫은 상태에서 비활성화 시키거나 해야함
 * 
 * 3- 인벤토리 셀렉팅 상태 전이 문제
 * 현재 잡화아이템을 타 인벤토리 슬롯으로 이동 시킬 때 
 * 스위칭시에는 상대 아이템을 이동시켜버리기 때문에 셀렉팅이 일어나지 않으며, 
 * 빈슬롯이동시에는 셀렉팅상태가 활성화되지 않아 다시 자기 셀렉팅이 일어나지 않는다.
 * 하지만, 잡화 아이템을 중첩시키는 경우 
 * 중첩수량이 남은 경우는 기존아이템 재셀렉팅을 하게된다. 하지만 그상태에서 다시 셀렉팅을하면?(셀렉팅 상태 테스트 해보기)
 * 중첩수량이 남지 않은 경우는 기존아이템이 파괴되고 중첩된 아이템이 셀렉팅되어버린다.
 * (파괴될때 상대인벤토리에 셀렉팅 상태는 전할 수 있지만 및 코루틴은 못돌린다.)
 * => 드롭시 상대 인벤토리에 상태를 전한다 vs 연결중인 인벤토리에 셀렉팅 상태를 전한다.
 * 
 * 
 * 
 * <2024_0222_최원준>
 * (수정내용)
 * 1. 플레이어 이동 시 아이템 셀렉팅이 발생하지 않기 위해 인벤토리창을 열고닫을 때 연결된 모든 열린 인벤토리(ex.퀵슬롯)의 모든 아이템의 기능을 스위칭 하도록 설정 
 * (열때만 기능을 복구하고 닫을 때는 차단하도록 설정)
 * 
 * 
 * (이슈)
 * 1. 인벤토리 창을 닫고 캐릭터를 움직일 때 가끔씩 인벤토리 내부의 아이템이 움직임.
 * => (해결완료)인벤토리창을 열고 닫을 때 캔버스그룹의 인터렉터블 속성도 같이 건드려주도록 설정하였음.
 * itemCG의 interactable속성만 건드려주면 버튼 작동을 하지 않게됨
 * 
 * 2- 인벤토리 내부에서 는 재셀렉팅이 안일어나지만 다른인벤토리로의 드랍시 재셀렉팅이 일어남
 * => 새로운 인벤토리로 이동할 때(다른 UI계층으로 이동할때) 아이템의 캔버스 그룹의 값이 초기화되는 특성이 있음
 * => (해결완료) AddItemToSlot메서드에서 OnItemAdded가 되면서 DimensionShift(false)가 일어나기 때문.
 * 인벤토리에서 타 인벤토리로 이동할때도 RemoveItem이후 다시 AddItem하게되면 isWorldPositioned가 true로 만들어진상태에서 이동해버리기 때문에 DimensionShift가 일어나게 됨.
 * 이를 구분하기 위해서 타인벤토리 이동시 RemoveItem하지말고 바로 AddItem을 하거나(불가함 - 문제는 등록상태기 때문에 스위칭시에는 IsEmptySlot에서 false가 일어나게 됨)
 * RemoveItem하면서 DimensionShift를 발생시키지 않고 2D상태 그대로 목록에서 제거하는 옵션을 추가로 만들어야 함.
 * => RemoveItem에서 등록해제옵션 isUnregister을 추가 선택인자로 만들어 해결
 * 
 * 3- 스왑시 상태창 남아있는 문제
 * => (해결완료) 슬롯드롭이 일어나기전 상대아이템의 상태창 종료메서드를 호출

 * 4- 스왑시 조건이 안맞아서 셀렉팅한 아이템이 원래자리로 돌아가는 경우에 클릭한지점의 상대아이템이 셀렉팅되는 문제 
 * (퀵슬롯의 무기창에 잡화아이템을 옮기면 잡화아이템은 되돌아가지만 무기가 셀렉팅되는 문제)
 * => (해결완료) ItemSelect스크립트에서 일시적으로 캔버스 그룹의 속성을 해제시킨 후 딜레이 시간 후 활성화 시키는 메서드를 슬롯 드롭 직전 호출해주는 형태로 구현
 * 
 * <2024_0223_최원준>
 * (이슈)
 * 1- 퀵슬롯의 상속으로인해 InventoryInfo 참조값이 두번 들어가있어서 레이캐스팅이 두번 들어가는 문제가 있음
 * => 동일 캔버스에 들어있는 퀵슬롯과 인벤토리를 연결해둔 상태이기 때문에 동일 캔버스 그래픽레이캐스터의 레이캐스팅이 2번발생하게됨.
 * 퀵슬롯과 인벤토리의 캔버스를 분리시키거나, 동일 캔버스에 둘때는 연결을 해제해야함.
 * (동일 캔버스에 둘 때 클라이언트 정보를 입력하면 2번 레이캐스팅이 들어가므로, 클라이언트 정보를 입력하지 말거나, 
 * 레이캐스팅 시 동일 캔버스 검사를 통하여 1번만 실행되는 등의 방법을 사용해야함)
 * => (해결완료)기존 Link형태는 그대로 두었으며, 결국 2번의 레이캐스팅이 일어나는 것은 캔버스 그룹이 동일하기 때문인데,
 * 이를 빼기에는 다른 캔버스에서 링크시킬 인벤토리가 존재할 때 코드의 일관성이 무너지기 때문에,
 * 단순히 결과 검출을 할 때 한번이라도 슬롯검사를 했을 때 break문을 써서 반복문을 종료하게 하였음.)
 * 
 * 
 * <2024_0224_최원준>
 * (이슈)
 * 1- 퀵슬롯 드랍조건 검사와 슬롯드랍조건 모두 검사가 완료된 후에 드랍을 진행해야하는 경우가 있음.
 * => 처음 드롭시에는 드롭할 아이템이 해당 인벤토리에 드랍될 수 있는지 검사를 해야하고, 스왑 시에는 스왑할 아이템이 드롭할 아이템이 기존에 담긴 인벤토리에 드랍될 수 있는지도 동시에 검사해야 함.
 * => (해결완료) 처음에는 슬롯드랍조건 검사와 퀵슬롯 드랍조건검사를 모두 통과시킨 후에 각 로직의 처리를 진행하였으나,
 * 중간 중간 (자식스크립트) 퀵슬롯으로 형변환 검사문이 계속 들어가서 코드가 난잡해버리는 경향이 있어,
 * 이를 하나로 통합하여 오버라이딩 해서 부모 조건과 자식 조건을 합쳐서 반환하는 형태로 구현해버림.
 * 
 * 
 * (2024_0210~2024_0224 작업내역 요약)
 * 1. 오토 세이브 로드 방식에서 지정 슬롯 세이브 로드 방식으로 변경
 * 2. 장착 아이템 세이브 로드 구현 (장착 메서드 통합)
 * 3. 장착 중인 무기 선택 시 장착 강제 해제되도록 구현 (더미이미지 버튼 및 더미이미지 셀렉팅 스크립트 구현)
 * 4. 인벤토리, 퀵슬롯 간 드롭 조건강화 (오버라이딩 통합 메서드 구현)
 * 5. 퀵슬롯-인벤토리 간 아이템 스왑 및 장착 무기 스왑 구현
 * 6. 캐릭터 이동 및 아이템 슬롯 드랍 시 셀렉팅 버그 대부분 해결
 * 7. 잡화아이템 추가 시 갯수 늘어나는 문제 해결
 * 7. 잡화아이템 옮길 때 지정 슬롯에 그대로 넣도록 구현 (지정슬롯 추가 메서드 구현)
 * 8. 잡화아이템 선택 후 동일 아이템 슬롯 드랍 시 중첩해서 채우기 기능 구현
 *  
 *   
 *  
 *  
